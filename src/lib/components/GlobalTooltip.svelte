<script lang="ts" context="module">
    ...
    // Create and export the tooltip store in module context so it's shared across all instances
...
export class ${project.title.replace(/[^a-zA-Z]/g, "")} {
    ...
    $: if (tooltipState) {
        console.log("GlobalTooltip state update:", tooltipState);
    }
    $: if (tooltipState.project) {
        codeSnippet = getEnhancedCodeSnippets(tooltipState.project);
        CategoryIcon =
            categoryIcons[
                tooltipState.project.category as keyof typeof categoryIcons
            ];
        console.log("GlobalTooltip project active:", {
            project: tooltipState.project.title,
            codeSnippet,
            CategoryIcon,
        });
    } else {
        console.log("GlobalTooltip no project");
    }

    let codeSnippet: CodeSnippet | null = null;
    let CategoryIcon: any = null;

    $: rarityColorClass = codeSnippet
...
{#if tooltipState.isVisible && tooltipState.project && codeSnippet}
    <div
        class="fixed z-[9999]"
        style="left: {tooltipState.position.x + 15}px; top: {tooltipState
            .position.y + 15}px;"
        role="tooltip"
        on:mouseenter={() => {
            console.log("Tooltip enter");
            tooltipStore.update((s) => ({ ...s, isInteracting: true }));
        }}
        on:mouseleave={() => {
        ...
        >
            ...
            >
                ...
                <div class="flex items-center gap-2 mb-2">
                    ...
                    />
                    <span class="text-sm font-bold {rarityColorClass}">
                        {codeSnippet.rarity.toUpperCase()}
                    </span>
                    {#if tooltipState.project.featured}
                ...
                <p class="text-sm text-slate-300">
                    {tooltipState.project.category} • {codeSnippet.language.toUpperCase()}
                </p>
            </div>
            ...
            <div class="p-4">
                ...
                <div class="space-y-2">
                    {#each Object.entries(codeSnippet.stats) as [stat, value]}
                        <div class="flex justify-between items-center">
                            <span class="text-xs text-slate-400">{stat}</span>
                ...
                <div class="mt-3 pt-3 border-t border-slate-600/30">
                    <div class="flex flex-wrap gap-1">
                        {#each tooltipState.project.technologies.slice(0, 6) as tech}
                            <span
